 ...
 * High-performance React implementation with strict accessibility and modular architecture.
 */
  ...
  return (
    <div className={themeClasses}>
      ...
      <main className="flex-1 grid grid-cols-1 lg:grid-cols-[320px_1fr_320px] gap-px bg-border overflow-hidden" role="main">
        ...
        <section className="bg-surface p-6 overflow-y-auto flex flex-col gap-6 border-r border-border" aria-label="Queues and Analytics">
          <VirtualQueue user={user} />
          <AnalyticsDashboard />
        </section>
        
        {/* Center: Live Map & Routing */}
        <section className="bg-surface p-6 overflow-y-auto flex flex-col gap-6" aria-label="Live Navigation">
        ...
        <section className="bg-surface p-6 overflow-y-auto border-l border-border" aria-label="AI Support">
      ...
      <footer className="bg-surface border-t border-border px-10 py-4 flex flex-wrap items-center gap-x-8 gap-y-2 text-[11px] text-text-sub" role="contentinfo">
        ...
        <div className="ml-auto flex items-center gap-4">
          <button 
            ...
            className="px-2 py-1 border border-border rounded hover:bg-bg transition-colors font-bold uppercase tracking-tighter"
            aria-pressed={isHighContrast}
          >
            {isHighContrast ? 'Standard Mode' : 'High Contrast'}
          </button>
          <div className="text-text-sub italic">
            FanFlow AI v1.2.0 • 2026
          </div>
        </div>
      </footer>
    </div>
